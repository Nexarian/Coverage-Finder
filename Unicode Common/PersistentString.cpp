...
void PersistentString::SetStringLength(unsigned long NewSize) {
	if (SizeOverride == true) {
		...
		OutputDebugString(L"Non-Fatal Error: Attempting to override PersistentString size without override enabled, override will be ignored\n");
	}
}
...
void PersistentString::ReadData(FileStream& File) {
	File >> SizeOverride;
	File >> OverriddenSize;
...
void PersistentString::WriteData(FileStream& File) const {
	File << SizeOverride;
	File << OverriddenSize;
...
/*
void PersistentString::WriteData(std::ostream& os) {
	os << SizeOverride;
	os << OverriddenSize;
	...
	if (SizeOverride == true) {
		std::string tmp = Str;
		os << tmp;
		os.WriteString(Str, OverriddenSize);
	}
	else {
		os << Str;
	}
...
void PersistentString::ReadData(std::istream& is) {
	is >> SizeOverride;
	is >> OverriddenSize;
